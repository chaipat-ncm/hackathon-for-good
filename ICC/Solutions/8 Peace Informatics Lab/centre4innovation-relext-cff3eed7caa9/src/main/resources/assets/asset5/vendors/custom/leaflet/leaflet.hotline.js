...
(function(root, plugin) {
  ...
  Hotline.prototype = {
    ...
    draw: function() {
      var ctx = this._ctx;
      ctx.globalCompositeOperation = "screen"; // todo: source-over
      ctx.lineCap = "round";
      if(this._outlineColor != 'none') { // todo: added
    ...
    _drawHotline: function(ctx) {
      ...
      const orgAlpha = ctx.globalAlpha;
      ctx.globalAlpha = 0.5;// todo: opacity
      for (i = 0, dataLength = this._data.length; i < dataLength; i++) {
        path = this._data[i];
        for (j = 1, pathLength = path.length; j < pathLength; j++) {
          pointStart = path[j - 1];
