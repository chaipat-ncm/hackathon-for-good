...
def get_df(path):
    ...
    df_vision = pd.DataFrame(d,index=['index'])
    ...
    for filename in os.listdir(path):
        ...
        if filename.endswith(".jpg") | filename.endswith(".jpeg") | filename.endswith(".png"):
            ...
            df_vision.loc[i] = {
             'img_id':filename+"_b" ,
             'texts':detect_text(f),
             'labels': detect_labels(f),
             'logos':detect_logos(f) ,
             'safe_search': detect_safe_search(f),
             'colors': detect_properties(f),
             'links': detect_web(f)}
...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = r"/Users/parvathykrishnank/Desktop/Hack4Peace/Hack4Peace.json"

def get_image_features_from_folders(ImageFolder_neg,ImageFolder_neutral):

    df_neg = get_df(ImageFolder_neg)
    df_neg['propoganda'] = 1
    df_neg.to_pickle('Processed/df_google_search_negative.pkl')
    df_neutral = get_df(ImageFolder_neutral)
    df_neutral['propoganda'] = 0
    df_neutral.to_pickle('Processed/df_google_search_neutral.pkl')
